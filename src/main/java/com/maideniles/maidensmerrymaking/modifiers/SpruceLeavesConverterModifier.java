...
public class SpruceLeavesConverterModifier extends LootModifier {
    ...
    public static final Supplier<Codec<SpruceLeavesConverterModifier>> CODEC = Suppliers.memoize(() -> RecordCodecBuilder.create(inst -> codecStart(inst)
            .and(ForgeRegistries.ITEMS.getCodec().fieldOf("addition").forGetter(m -> m.addition))
            .apply(inst, SpruceLeavesConverterModifier::new)
    ));
    public SpruceLeavesConverterModifier(LootItemCondition[] conditionsIn, Item addition) {
        super(conditionsIn);
