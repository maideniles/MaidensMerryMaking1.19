...
public class EasterChestLootModifier extends LootModifier {
    ...
    public static final Supplier<Codec<EasterChestLootModifier>> CODEC = Suppliers.memoize(() -> RecordCodecBuilder.create(inst -> codecStart(inst)
            .and(ForgeRegistries.ITEMS.getCodec().fieldOf("addition").forGetter(m -> m.addition))
            .apply(inst, EasterChestLootModifier::new)
    ));
    public EasterChestLootModifier(LootItemCondition[] conditionsIn, Item addition) {
        super(conditionsIn);
